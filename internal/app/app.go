...
func Run(cfg *config.Config) {
	...
	httpServer := initializeRPC(l, queryRawTransactionUseCase, cfg)

	ShutdownApplicationHandler(l, httpServer, redisClient)
}
...
func initialRedisClient(cfg *config.Config) *redis.RedisCache {
	redisCache := redisCache.NewRedisClient(redis.Addr(cfg.REDIS.Addr), redis.Db(cfg.REDIS.DB), redis.Password(cfg.REDIS.Password))
	return redisCache
}
